...
export default function StudyDeckPage({ params }: StudyDeckPageProps) {
  const { id } = params;
  ...
  const fetchDueCards = async () => {
    try {
      ...
      setDeckTitle(deckData.deck?.title || 'Deck');

      // Fetch due cards
      const response = await fetch(`/api/decks/${id}/due`);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-ink-600">Loading cards...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-ink-50">
      ...
      <header className="border-b border-ink-200 bg-white">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
          <Link href={`/decks/${id}`} className="btn-ghost">
            <ArrowLeft className="mr-2 h-4 w-4" />
            Back to Deck
          </Link>

          <h1 className="text-lg font-semibold text-ink-900">{deckTitle}</h1>
